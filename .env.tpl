#
# SPDX-FileCopyrightText: 2023 INFO.nl
# SPDX-License-Identifier: EUPL-1.2+
#

# This file contains the environment variables that are used when running ZAC locally by the ICATT ZAC team.
# It is used when you run ZAC locally from IntelliJ (see: docs/development/ideConfig.md), but not when you run ZAC from Docker Compose itself.

# To use this file you need to use the 1Password CLI extensions and you need to have access to the corresponding INFO/ICATT 1Password vault.
# Please see .env.example for descriptions of the environment variables.

# -------------------------
# ZAC environment variables
# -------------------------

AUTH_ENABLE_PKCE=true
AUTH_REALM=zaakafhandelcomponent
AUTH_RESOURCE=zaakafhandelcomponent
AUTH_SECRET=keycloakZaakafhandelcomponentClientSecret
AUTH_SERVER=http://localhost:8081
AUTH_SSL_REQUIRED=none
BAG_API_CLIENT_MP_REST_URL=op://Dimpact/ZAC-.env-$APP_ENV/BAG/API_CLIENT_MP_REST_URL
BAG_API_KEY=op://Dimpact/ZAC-.env-$APP_ENV/BAG/API_KEY
BRON_ORGANISATIE_RSIN=123443210
BRP_ORIGIN_OIN=123443210
BRP_API_CLIENT_MP_REST_URL=http://localhost:5010/haalcentraal/api/brp
BRP_PROTOCOLLERING=iConnect
BRP_DOELBINDING_ZOEKMET=BRPACT-Totaal
BRP_DOELBINDING_RAADPLEEGMET=BRPACT-Totaal
BRP_VERWERKINGSREGISTER=Algemeen
BRP_VERWERKINGSREGISTER_EXT_WITH_ZAAKTYPE=true
CONTEXT_URL=http://localhost:8080
DB_HOST=localhost:54320
DB_NAME=zac
DB_PASSWORD=password
DB_USER=zac
GEMEENTE_CODE=0001
GEMEENTE_MAIL=op://Dimpact/ZAC-.env-$APP_ENV/GEMEENTE/MAIL
GEMEENTE_NAAM=FakeGemeente
KEYCLOAK_ADMIN_CLIENT_ID=zaakafhandelcomponent-admin-client
KEYCLOAK_ADMIN_CLIENT_SECRET=zaakafhandelcomponentAdminClientSecret
KLANTINTERACTIES_API_CLIENT_MP_REST_URL=http://localhost:8002
KLANTINTERACTIES_API_TOKEN=fakeToken
KVK_API_CLIENT_MP_REST_URL=op://Dimpact/ZAC-.env-$APP_ENV/KVK/API_CLIENT_MP_REST_URL
KVK_API_KEY=op://Dimpact/ZAC-.env-$APP_ENV/KVK/API_KEY
OBJECTS_API_CLIENT_MP_REST_URL=http://host.docker.internal:8010
OBJECTS_API_TOKEN=fakeZacObjectsToken
OBJECTTYPES_API_CLIENT_MP_REST_URL=http://host.docker.internal:8011
OBJECTTYPES_API_TOKEN=fakeZacObjectTypesToken
OFFICE_CONVERTER_CLIENT_MP_REST_URL=http://localhost:8083
OPA_API_CLIENT_MP_REST_URL=http://localhost:8181
OPEN_NOTIFICATIONS_API_SECRET_KEY=openNotificatiesApiSecretKey
PABC_API_CLIENT_MP_REST_URL=http://host.docker.internal:8006
PABC_API_KEY=zac-test-api-key
EPISTOLA_CLIENT_MP_REST_URL=op://Dimpact/Epistola Client MP REST URL/credential
EPISTOLA_TENANT_ID=op://Dimpact/Epistola Tenant Id/credential
EPISTOLA_CATALOG_ID=op://Dimpact/Epistola Catalog Id/credential
EPISTOLA_CLIENT_API_KEY=op://Dimpact/Epistola Client API Key/credential
SMARTDOCUMENTS_ENABLED=true
SMARTDOCUMENTS_AUTHENTICATION=op://Dimpact/ZAC-.env-$APP_ENV/SMARTDOCUMENTS/AUTHENTICATION
SMARTDOCUMENTS_CLIENT_MP_REST_URL=op://Dimpact/ZAC-.env-$APP_ENV/SMARTDOCUMENTS/CLIENT_MP_REST_URL
SMARTDOCUMENTS_FIXED_USER_NAME=op://Dimpact/ZAC-.env-$APP_ENV/SMARTDOCUMENTS/SMARTDOCUMENTS_FIXED_USER_NAME
SMARTDOCUMENTS_WIZARD_AUTH_ENABLED=true
SMTP_SERVER=op://Dimpact/ZAC-.env-$APP_ENV/SMTP/SERVER
SMTP_PORT=op://Dimpact/ZAC-.env-$APP_ENV/SMTP/PORT
SMTP_USERNAME=op://Dimpact/ZAC-.env-$APP_ENV/MAILJET/API_KEY
SMTP_PASSWORD=op://Dimpact/ZAC-.env-$APP_ENV/MAILJET/API_SECRET_KEY
SIGNALERINGEN_DELETE_OLDER_THAN_DAYS=14
SOLR_URL=http://localhost:8983
VERANTWOORDELIJKE_ORGANISATIE_RSIN=316245124
ZAC_INTERNAL_ENDPOINTS_API_KEY=fakeZacInternalEndpointsApiKey
ZGW_API_CLIENT_MP_REST_URL=http://host.docker.internal:8001/
ZGW_API_CLIENTID=zac_client
ZGW_API_SECRET=openzaakZaakafhandelcomponentClientSecret

# -----------------------------------------
# e2e only environment variables
# -----------------------------------------
E2E_TEST_USER_1_USERNAME=op://Dimpact/e2etestuser1/username
E2E_TEST_USER_1_PASSWORD=op://Dimpact/e2etestuser1/password
E2E_TEST_USER_2_USERNAME=op://Dimpact/e2etestuser2/username
E2E_TEST_USER_2_PASSWORD=op://Dimpact/e2etestuser2/password
ZAC_URL=op://Dimpact/zaakafhandelcomponent-zac-dev/website
OPEN_FORMS_URL=op://Dimpact/open-formulieren-zac-dev/website

# -----------------------------------------
# Docker Compose only environment variables
# -----------------------------------------
DOCKER_COMPOSE_RAADPLEGER_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/RAADPLEGER_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_RAADPLEGER_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/RAADPLEGER_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_BEHANDELAAR_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/BEHANDELAAR_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_BEHANDELAAR_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/BEHANDELAAR_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_COORDINATOR_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/COORDINATOR_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_COORDINATOR_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/COORDINATOR_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_RECORDMANAGER_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/RECORDMANAGER_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_RECORDMANAGER_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/RECORDMANAGER_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_BEHEERDER_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/BEHEERDER_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_RAADPLEGER_EN_BEHANDELAAR_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/RAADPLEGER_EN_BEHANDELAAR_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_ZAAKSPECIFIEK_AUTORISATIE_BEHANDELAAR_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/ZAAKSPECIFIEK_AUTORISATIE_BEHANDELAAR_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_RAADPLEGERS_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_RAADPLEGERS_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_RAADPLEGERS_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_RAADPLEGERS_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_BEHANDELAARS_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_BEHANDELAARS_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_BEHANDELAARS_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_BEHANDELAARS_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_COORDINATORS_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_COORDINATORS_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_COORDINATORS_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_COORDINATORS_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_RECORDMANAGERS_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_RECORDMANAGERS_TEST_1_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_RECORDMANAGERS_TEST_2_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_RECORDMANAGERS_TEST_2_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_BEHEERDERS_ELK_DOMEIN_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_BEHEERDERS_ELK_DOMEIN_EMAIL_ADDRESS
DOCKER_COMPOSE_GROUP_ZAAKSPECIFIEK_AUTORISATIE_BEHANDELAARS_TEST_1_EMAIL_ADDRESS=op://Dimpact/ZAC-.env-$APP_ENV/DOCKER_COMPOSE/GROUP_ZAAKSPECIFIEK_AUTORISATIE_BEHANDELAARS_TEST_1_EMAIL_ADDRESS
